...
## omp host (read this first on this fork)

i still want the same two benny behaviors: triage slack issue reports, then reproduce and fix confirmed bugs with real ui evidence. the `SKILL.md` files in this pack stay direct automation instructions, not slash skills.

if the host is **Cursor**, keep the cursor automations path below (`/automate`, `.cursor/settings.json`, slack-triggered live automations).
