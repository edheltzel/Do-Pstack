...
Benny needs external configuration and, on Cursor, two live Cursor automations.
...
On omp, skip this section's `/automate` path. There is no Automations editor. After configuration is committed, run the operational files in an omp session when a report arrives. See HOST GAP above.
