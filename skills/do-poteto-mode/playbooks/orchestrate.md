...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through omp `task({ context, tasks: [{ agent, task }] })`. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3, and a nested spawn uses the same omp `task` schema: `agent` + `task`, no `environment` or `model` field). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Spawn with `task` and `agent: "poteto-agent"` or `agent: "swarm-worker"`. `environment: cloud` is Cursor-only and skipped on omp. Use a local worker when the task needs this machine: reading local transcripts under `agent-transcripts/`; simulators and local IDE state; auth that exists only here. `control-ui` / `control-cli` from `cursor-team-kit` are Cursor-only and skipped on omp. Briefs inline what a worker needs or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). One agent definition, one role: a different model family for the verifier is not available unless the user changes that agent's `model` in `/agents`.
