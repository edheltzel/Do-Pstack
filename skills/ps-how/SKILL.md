...
Spawn all explorers in one `task` call with multiple `tasks` entries (same agent name, different task text). Do not pass `readonly` or `model`.

```
task({ context: "…", tasks: [
  { agent: "how-explorer", task: "<angle 1 + explorer prompt>" },
  { agent: "how-explorer", task: "<angle 2 + explorer prompt>" }
] })
```
...
Call `task` with `tasks: [{ agent: "how-explainer", task: "<question + explainer prompt>" }]`. Do not pass `readonly` or `model`.
...
Once all explorers return, call `task` with `tasks: [{ agent: "how-explainer", task: "<findings + explainer prompt>" }]`. Do not pass `readonly` or `model`.
