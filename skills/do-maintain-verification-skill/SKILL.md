---
name: do-maintain-verification-skill
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map. Look in `.omp/skills/verify-*/` first. Mention `.cursor/skills/verify-*/` only as a legacy path to migrate into `.omp/skills/`. Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target.
