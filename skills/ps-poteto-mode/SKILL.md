...
- `/deslop`, Graphite, Bugbot, and `cursor-team-kit` (`control-cli`, `control-ui`) are Cursor-only and skipped on omp.
...
- **Babysit.** Driving a PR or a stack to merge-ready: conflicts, review threads, CI. Stops before merge. `playbooks/babysit.md`.
